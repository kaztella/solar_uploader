...
#include "solar_uploader.h"

int cc_count = 0;
CC_DATA *cclist=NULL;
DB_DATA *db_data=NULL; //db data 
...
int main(int argc, char **argv) {
	...
	while (1) {
//		if (debug) printf("main looping.....\n");
		if (debug) printf("%s \n",last_upload_time=get_last_upload_time(conn));
		printf("internet connection =%d\n",chk_upload_ip("52.70.70.126")); //now constructing	
		gather_data_all(); //get modbus data
		if (write_db) write_to_db(conn);
//		read_from_db(conn); //now constructing
...
//		if (debug) printf("%d\n",c);
		if (c == 2) {
			...
			if (!j) {
				...
				sleep(poll_interval);
				connect_all();
				if (debug) printf("connect(1)\n");
			} else if (j<cc_count) {
				connect_all();
				if (debug) printf("connect(2)\n");
			}
			c=0;
		} else {
			sleep(poll_interval);
...
void write_to_db(PGconn *conn) {
	...
	for(i=0;i<cc_count;i++) {
		if (cclist[i].alive) {
			battery_volts = (float)cclist[i].modbus_register[14]/10.0f; //4115 average battery volts (1s)
			pv_volts = (float)cclist[i].modbus_register[15]/10.0f; //4116
			battery_volts_raw = (float)cclist[i].modbus_register[175]/10.0f; //4276 battery volt unfiltered
			pv_volts_raw = (float)cclist[i].modbus_register[176]/10.0f; //4277 
			battery_amps = (float)cclist[i].modbus_register[16]/10.0f; //4117 Average battery current (1s)
			pv_amps = (float)cclist[i].modbus_register[20]/10.0f; //4121 average pv terminal input current(1s)
			pv_voc = (float)cclist[i].modbus_register[21]/10.0f; //4122 last measured open circuit voltage at PV terminal input
			watts = cclist[i].modbus_register[18]; //4119 average power to battery
			kWh_today = (float)cclist[i].modbus_register[17]/10.0f; //4118 average energy to the batery. this is reset once per day 
			Ah_today = cclist[i].modbus_register[24]; // 4125 dayli Amp hours reset at 23:59
			ext_temp = (float)cclist[i].modbus_register[31]/10.0f; //4132  tmp sensor op. (battery temp)
			int_fet_temp = (float)cclist[i].modbus_register[32]/10.0f; //4133 temp a power FET
			int_pcb_temp = (float)cclist[i].modbus_register[33]/10.0f; //4134 
			life_kWh =    (float)((unsigned long)cclist[i].modbus_register[25] + ((unsigned long)cclist[i].modbus_register[26]<<16))/10.0f;
			life_Ah = (unsigned long)cclist[i].modbus_register[27] + ((unsigned long)cclist[i].modbus_register[28]<<16);
			float_seconds_today = cclist[i].modbus_register[37];
			combochargestate = cclist[i].modbus_register[19]; // 4120  0:rest, 3:Absorb, 4:Bulk, 5:Float, 6:FloatMPPT, 7:EQ, 10:HyperVoc, 18:EQMPPT


			if (f) { //f is 0 at first.
				strcat(sql,", ");
			}
			f++; //f incremented to 1 here
			// sql contains the "insert command"
			...
			strcat(sql,sql2); //continue to add data after data over all controller

		}
	}
    //Do SQL
	if (f) { //if f=0 Don't do this SQL and exit . I can't understand if there are any cases f become 0 here..
		if (debug) { printf(sql); printf("\n"); }
		res = psql_query(conn, sql); //res check
		if (!res) { 
...
//addr
	...
	n=id[1]&0xff;
	len=read(cclist[cc].s, &cclist[cc].modbus_register[offset], n); //n bytes read from cclist[cc].s file  and write to .modbus_register[]
	if (len != n) {
		// problem reading...
		if (debug) printf("couldn't read from %d, modbus issue %d %d (c)\n",cc,id[0],id[1]);
		cclist[cc].alive = 0;
...
void connect_all() {
	...
	for(i=0;i<cc_count;i++) {
		...
		if (!cclist[i].alive) {
			if (debug) printf("alive=0@Ln690\n");
			if (!cclist[i].hp) {
				if (debug) printf("cclist.hp=0@Ln692\n");
...
void read_from_db(PGconn *conn) {
	...
	time_t ts;
	PGresult *res;
	int cid_fnum,battery_volts_fnum, pv_volts_fnum, battery_volts_raw_fnum, pv_volts_raw_fnum, battery_amps_fnum, pv_amps_fnum, pv_voc_fnum, kWh_today_fnum, ext_temp_fnum, int_fet_temp_fnum, int_pcb_temp_fnum, life_kWh_fnum,watts_fnum, Ah_today_fnum, life_Ah_fnum, float_seconds_today_fnum, combochargestate_fnum;
  
	char sql[50000];
	char sql2[5000];
	
	if (!last_upload_time){
		if (debug) printf("update_time=%s\n",last_upload_time);
		last_upload_time=get_last_upload_time(conn);
	}	
	
//	strcpy(sql,"select cid, ts,battery_volts, pv_volts, battery_volts_raw, pv_volts_raw, battery_amps, pv_amps, pv_voc, watts, kWh_today, Ah_today, ext_temp, int_fet_temp, int_pcb_temp, life_kWh, life_Ah, float_seconds_today, combochargestate from charge_controller_data order by pv_volts DESC");
	sprintf(sql,"select SUBSTRING(CAST(ts AS varchar),1,15)|| '0:00' AS ts_stat, AVG(battery_volts) AS avg_battery_volts,AVG(battery_amps) AS avg_battery_amps,AVG(pv_volts)AS avg_pv_volts,AVG(pv_amps)AS avg_pv_amps FROM charge_controller_data  where ts>=cast('%s' AS timestamp) AND ts< cast('%s' as timestamp)+ cast('10 minute' as interval) GROUP BY ts_stat",last_upload_time,last_upload_time);

	res = psql_query(conn, sql); 
	if (!res) {
			if (debug)printf("Error doing SQL select!\n");
		}
	// fnum get	
		
	cid_fnum=PQfnumber(res,"cid");	
	battery_volts_fnum=PQfnumber(res,"battery_volts");
	pv_volts_fnum=PQfnumber(res,"pv_volts");
	battery_amps_fnum=PQfnumber(res,"battery_amps");
	
	
	if (debug) printf("%d,%d\n",cid_fnum,battery_volts_fnum);
	// sae data to var
	for(i=0;i<cc_count;i++) {
			cid				=atoi(PQgetvalue(res,1,cid_fnum));
			battery_volts 	=atof(PQgetvalue(res,1,battery_volts_fnum));
			pv_volts 			=atof(PQgetvalue(res,1,pv_volts_fnum));
	//		battery_volts_raw = (float)cclist[i].modbus_register[175]/10.0f;
	//		pv_volts_raw = (float)cclist[i].modbus_register[176]/10.0f;
			battery_amps = (float)cclist[i].modbus_register[16]/10.0f;
			pv_amps = (float)cclist[i].modbus_register[20]/10.0f;
...
/*
			...
			f++;
*/
			sprintf(sql2,"(%d, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %u, %.1f, %u, %.1f, %.1f, %.1f, %.1f, %u, %u, %u)",cid, battery_volts, pv_volts, battery_volts_raw, pv_volts_raw, battery_amps, pv_amps, pv_voc, watts, kWh_today, Ah_today, ext_temp, int_fet_temp, int_pcb_temp, life_kWh, life_Ah, float_seconds_today, combochargestate);
			strcat(sql,sql2);
			printf(sql);
	}
...
// 
char* get_last_upload_time(PGconn *conn){
	int cid;
	PGresult *res;
	char sql[50000];
	char sql2[5000];
//	strcpy(sql,"SELECT MAX(ts_stat) FROM uploaded_stat_data"); 
//	res = psql_query(conn, sql); 
	if (!res) {
		if (debug)printf("Error doing SQL select!\n");
	}
...
{
	strcpy(sql,"SELECT MIN(ts) FROM charge_controller_data"); 
	res = psql_query(conn, sql); 		
	if (!res) {
		if (debug)printf("Error doing SQL select!\n");
	}
...
int chk_upload_ip(char *ip_addr){
#define BUF 64
#define CHECKCOUNT 1

int result;
char command[BUF];
//char *hostptr = "192.168.0.0";

sprintf(command, "ping -q -c %d %s >/dev/null", CHECKCOUNT, ip_addr);
